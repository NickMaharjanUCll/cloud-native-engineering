...
let mock_orderDb_getOrdersByCustomerUsername: jest.Mock;
...
beforeEach(() => {
    ...
    mock_orderDb_getOrdersByCustomerUsername = jest.fn();
});
...
test("Given logged in customer; When calling getOrdersByCustomerUsername; Then orders from that customer are returned.", async () => {
    // GIVEN
    // Variables at the top of this file.
    customerDb.getCustomerByUsername = mock_customerDb_getCustomerByUsername.mockReturnValue(customer);
    orderDb.getOrdersByCustomerId = mock_orderDb_getOrdersByCustomerUsername.mockReturnValue([order]);

    // WHEN
    const result = await orderService.getOrdersByCustomerUsername(auth);
    ...
    expect(mock_orderDb_getOrdersByCustomerUsername).toHaveBeenCalledTimes(1);
    expect(mock_orderDb_getOrdersByCustomerUsername).toHaveBeenCalledWith(customer.getId());

});

test("Given logged in admin; When calling getOrdersByCustomerUsername; Then unauthorized exception is thrown.", async () => {
    // GIVEN
    // Variables at the top of this file.
    const admin: Auth = { username: "ohYeah", role: "admin"}

    // WHEN
    const getOrdersByCustomerUsername = () => orderService.getOrdersByCustomerUsername(admin);

    // THEN
    expect(getOrdersByCustomerUsername).rejects.toThrow("You are not authorized to access this resource.");
});

test("Given logged in guest; When calling getOrdersByCustomerUsername; Then unauthorized exception is thrown.", async () => {
    // GIVEN
    // Variables at the top of this file.
    const guest: Auth = { username: "ehh", role: "guest"}

    // WHEN
    const getOrdersByCustomerUsername = () => orderService.getOrdersByCustomerUsername(guest);

    // THEN
    expect(getOrdersByCustomerUsername).rejects.toThrow("You are not authorized to access this resource."); 
});

test("Given logged in customer whose username does not exist in the database; When calling getOrdersByCustomerUsername; Then service exception is thrown.", async () => {
    // GIVEN
    // Variables at the top of this file.
    customerDb.getCustomerByUsername = mock_customerDb_getCustomerByUsername.mockReturnValue(null);
    const customer: Auth = { username: "im no actually in the database ooops", role: "customer"}

    // WHEN
    const getOrdersByCustomerUsername = () => orderService.getOrdersByCustomerUsername(customer);

    // THEN
    expect(getOrdersByCustomerUsername).rejects.toThrow("Customer does not exist"); 
});
